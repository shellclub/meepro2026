...
export default function AdminProductEdit({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return <div style={{ textAlign: "center", padding: "60px", color: "#64748b", fontSize: "16px" }}>กำลังโหลดข้อมูลสินค้า...</div>;
  }
  ...
  return (
    <div style={{ maxWidth: "900px" }}>
      <div style={{ marginBottom: "25px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h1 style={{ fontSize: "24px", fontWeight: "700", color: "#1e293b", margin: 0 }}>✏️ แก้ไขสินค้า</h1>
          <p style={{ color: "#64748b", margin: "4px 0 0 0", fontSize: "14px" }}>{form.name}</p>
        </div>
        <button onClick={() => router.back()} style={{ padding: "8px 16px", background: "#f1f5f9", color: "#64748b", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "13px" }}>← กลับ</button>
      </div>
